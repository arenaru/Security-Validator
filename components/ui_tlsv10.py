...
def render_tlsv10_card(hasil_tlsv10):
    st.subheader("🔒 TLS 1.0 Detection")
    
    if hasil_tlsv10:
        df = pd.DataFrame(hasil_tlsv10)
        ...
        if 'status' in df.columns:
            ...
            if insecure_count > 0:
                ...
                st.success("✅ All TLS 1.0 Disabled (Secure)")
    else:
        st.warning("No TLS 1.0 scan data available or scan failed")
